...
| #   | Task                              | Rationale                                                 |
| --- | --------------------------------- | --------------------------------------------------------- |
| 3.1 | Payment integration + plan limits | The 50-doc free tier is hardcoded with no billing.        |
| 3.2 | PostgreSQL `UserStore` adapter    | SQLite does not scale horizontally.                       |
| 3.3 | Distributed `KebStore` adapter    | Filesystem store races across instances.                  |
| 3.4 | Admin dashboard                   | `/api/status` JSON is not enough for ops.                 |
| 3.5 | Password recovery                 | No email means no reset; forgotten passwords = data loss. |
| 3.6 | GDPR export / account deletion    | Required for EU users.                                    |
...
1. **Launch scope:** Do we ship local-only first, or is hosted multi-user required at launch?
1. **Billing:** Is Stripe the preferred payment provider?
1. **Domain:** Is `keb.mdevd.co` the final public domain for landing + help pages?
